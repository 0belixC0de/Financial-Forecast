...
/**
 * A transparent, price-based "market mood" signal. A free static site can't
 * securely call an LLM over live news, so instead we read momentum directly
 * from the price action: short vs long moving average plus recent drift.
 */
export function momentumSentiment(closes: Candle[]): {
  sentiment: Sentiment;
  reason: string;
} {
  const s = closes.map((c) => c.close).filter((v) => v > 0);
  if (s.length < 25) {
    return { sentiment: "neutral", reason: "Not enough history for a momentum read." };
  }
  ...
  const short = sma(10);
  const long = sma(50 > s.length ? s.length : 50);
  const monthAgo = s[Math.max(0, s.length - 21)];
  const monthChange = ((last - monthAgo) / monthAgo) * 100;

  const aboveTrend = short > long;
  const score = (aboveTrend ? 1 : -1) + (monthChange > 2 ? 1 : monthChange < -2 ? -1 : 0);

  if (score >= 2) {
    return {
      sentiment: "bullish",
      reason: `Price is above its trend and up ${monthChange.toFixed(1)}% over the last month.`,
    };
  }
  if (score <= -2) {
    return {
      sentiment: "bearish",
      reason: `Price is below its trend and down ${Math.abs(monthChange).toFixed(1)}% over the last month.`,
    };
  }
  return {
    sentiment: "neutral",
    reason: `Mixed signals — ${aboveTrend ? "above" : "below"} trend, ${monthChange >= 0 ? "+" : ""}${monthChange.toFixed(1)}% this month.`,
  };
}
